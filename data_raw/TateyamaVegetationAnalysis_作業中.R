...
#'
#' par(mfrow=c(3,4))
#' for(i in plt$plot_name[-c(4,9)])Fig_DominanceValue(i)
#'
#' par(mfrow=c(1,1))
#'
...
d %>%
  ...
  group_by(layer) %>%
  summarise(
    across(starts_with("di"), sum, na.rm = TRUE),
    .groups = "drop"
  )
...
# 1~5期出現した種を解析対象とする
i_calc <- which(rowSums(d[,paste0("f",1:5)]>0)==5)
sp_calc <- d$sp[i_calc]
d[i_calc,]

# 総合優占度の集計
dom<-d[i_calc,paste0("di",1:5)]
dom
tapply()
...
#'
#' FieldNote_CheckCorrect(d_Joudosan)
#'
#'
