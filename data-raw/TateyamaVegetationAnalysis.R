...
#'　
#'　
EncodingExchange<-function(filename="H30植生調査野帳_美松.csv",en_old="cp932",en_new="utf-8"){
...
#'
#' @examples
#' SpeciesNameCheck(c("ブナ","スギ","タブノキ"))
#' d<-FieldNote_Arimine2019_raw
#' head(d)
#' SpeciesNameCheck(d$種名)
#'
#' SpeciesNameCheck(vv$sp)
#'
#'
#'
SpeciesNameCheck<-function(spj){
  ...
  if(anyNA(fl.i)){
    cat(spj[is.na(fl.i)], "はリストにありません。¥n種名を修正するか、floraに新たな種を追加してください。¥n")
  } else {    cat("入力されている種名はすべて目録に含まれています。¥n")}
...
# # class check ####
#   d$階層<-sptype$階層[match(flora$form[fl.i],sptype$コード)]
#   d<-d[order(d$サブプロット,d$サブプロット,d$階層,d$種名),]
...
analysis000_FieldNoteInput <- function(InputFileName="H31植生調査野帳_有峰.csv",OutputFileName="06美松-植生野帳2018.csv"){
  ...
  setwd("‾/8T/Dropbox/00D/00/立山植生モニタリング事業/第05期/2025/TateyamaVegetation/data-raw/2025/有峰")
...
#
#
# # yachou<-"09浄土-植生野帳2024.csv"   #### デバック前
# # yachou2<-"09浄土2024.csv" 　#### デバック後
#
# yachou<-"立山植生(04)植生-上ノ小平-2024.csv"   #### デバック前
# yachou2<-"04上ノ小平2024.csv" 　#### デバック後
#
#
# ## 野帳データファイル読み込み ####
