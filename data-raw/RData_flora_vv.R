...
# Data Backup ####
#　write.csv(flora,file="data-raw/flora_old.csv")　####　これまでのdata-raw/flora_old.csvとして保存
#  save(flora,file="data-raw/flora_old.RData")
...
# write.csv(df,"data-raw/flora_edit.csv",row.names = F)

df<-readr::read_csv("data-raw/flora_edit.csv")
df[order(df$LAPG.no.),]
unique(df$form)

flora4<-flora
# save(flora4,file="data-raw/flora4.RData")
